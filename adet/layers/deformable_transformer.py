...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed, text_embed, text_pos_embed, queries, query_pos_embed,
                text_mask=None):
        ...
        # encoder
        memory = self.encoder(src_flatten, spatial_shapes, level_start_index, valid_ratios, queries,
        ...
        hs, hs_text, inter_references = self.decoder(
            ...
            level_start_index, valid_ratios, queries, query_pos_embed, query_pos, text_pos_embed, mask_flatten, text_mask
        )
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        # cross attention between the feats and the queries
        self.attn_cross_query = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
        self.dropout_cross_query = nn.Dropout(dropout)
        self.norm_cross_query = nn.LayerNorm(d_model)
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, query,
                padding_mask=None):
        # self attention
        ...
        src2 = self.attn_cross_query(src.swapdims(0, 1), query[:, 0].swapaxes(0, 1),
                                     query[:, 0].swapaxes(0, 1), None)[0].swapdims(0, 1)
        src = src + self.dropout_cross_query(src2)
        src = self.norm_cross_query(src)
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, queries, pos=None, padding_mask=None):
        output = src
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device)
        for _, layer in enumerate(self.layers):
            output = layer(output, pos, reference_points, spatial_shapes,
                           level_start_index, queries, padding_mask)
...
class DeformableCompositeTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        # cross attention for text and the queries :)
        self.attn_cross_text_query = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
        self.dropout_cross_text_query = nn.Dropout(dropout)
        self.norm_cross_text_query = nn.LayerNorm(d_model)
    ...
    def forward(self, tgt, query_pos, tgt_text, query_pos_text, reference_points, src, src_spatial_shapes,
                level_start_index, queries, query_pos_embed, src_padding_mask=None, text_padding_mask=None):
        ## input size
        ...
        # tgt_text:           batch_size, n_objects, n_chars, embed_dim
        # query_pos_text:     batch_size, n_objects, n_chars, embed_dim

        # cross loc-query
        tgt_query = self.attn_cross_loc_query(
            self.with_pos_embed(tgt, query_pos).flatten(0, 1).transpose(0, 1),
            self.with_pos_embed(queries, query_pos_embed).flatten(0, 1).transpose(0, 1),
            queries.flatten(0, 1).transpose(0, 1),
        ...
        # text branch - cross attention with the queries and the queries
        tgt_query = self.attn_cross_text_query(
            self.with_pos_embed(tgt_text, query_pos_text).flatten(0, 1).transpose(0, 1),
            self.with_pos_embed(queries, query_pos_embed).flatten(0, 1).transpose(0, 1),
            queries.flatten(0, 1).transpose(0, 1),
            None)[0].transpose(0, 1).reshape(tgt_text.shape)
        tgt_text = tgt_text + self.dropout_cross_text_query(tgt_query)
        tgt_text = self.norm_cross_text_query(tgt_text)
...
class DeformableCompositeTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, tgt_text, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
                queries, query_pos_embed, query_pos=None, query_pos_text=None, src_padding_mask=None, text_padding_mask=None):
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output, output_text = layer(output, query_pos, output_text, query_pos_text, reference_points_input, src,
                                        src_spatial_shapes, src_level_start_index, queries, query_pos_embed,
                                        src_padding_mask, text_padding_mask)
