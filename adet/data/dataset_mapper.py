...
from adet.utils.queries import text_to_query_t1, text_to_query_t2, text_to_query_t3, compare_queries, indices_to_text
...
class DatasetMapperWithBasis(DatasetMapper):
    ...
    def __call__(self, dataset_dict):
        ...
        if len(dataset_dict["annotations"]):
            target = random.choice(dataset_dict["annotations"]) # We select a random annotation
            ...
            for anno in dataset_dict["annotations"]:
                # Compare the regex with the text using regular expressions
                try:
                    if re.fullmatch(regex, indices_to_text(anno['rec'])):
                        anno["query"] = torch.tensor(target_query)
